...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get('*all', (req, res) => {
      ...
      // Inject API key from Cloud Run environment into the page at request time
      const apiKey = process.env.API_KEY || process.env.GEMINI_API_KEY || "";
      const injection = `<script>window.__API_KEY__="${apiKey}";window.__GEMINI_API_KEY__="${apiKey}";</script>`;
